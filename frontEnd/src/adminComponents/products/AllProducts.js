...
function AllProducts() {
  ...
  let GetProducts = bindActionCreators(getProductsForAdmins,  useDispatch())
  let DeleteProduct = bindActionCreators( deleteProduct, useDispatch())
          ...
          <table className="table table-striped table-bordered my-4">
            ...
            <tbody>
              ...
              <tr key={product._id}>
                ...
                <td>{product.price}$</td>
                <td>{product.publisher?product.publisher.name:'Deleted user'}</td>
                <td>
                  <i style={{cursor:'pointer'}} onClick={()=>handleDelete(product._id)} className='far fa-times-circle text-danger mr-2'></i>
  ...
  return (
    <div className='products container text-center' style={{minHeight:'200px'}}>
      <h3 className='text-center text-info mt-4'>Products Show</h3>
        ...
        <Link to={'/admin-add-product'} className="btn btn-sm btn-outline-dark my-4 float-right">
