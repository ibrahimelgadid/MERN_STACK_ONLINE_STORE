...
router.post('/addtocart', passport.authenticate('jwt', {session:false}),(req, res) =>{
  ...
  .then(cart=>{
    ...
    if(!cart){
      ...
      newCart.save((err,done)=>{
        if (err) { console.log(err) }
        return res.status(201).json(done)
      })
    }else{
      let productIndex = cart.selectedProduct.map(c=>c._id).indexOf(req.body.id);
      ...
      if(productIndex <0){
        ...
        Cart.findByIdAndUpdate({_id:cartID}, {$set:cart},{new:true}, (err,done)=>{
          if (err) { console.log(err) }
          return res.status(201).json(done)
        })
...
//---------------------------------------------|
router.post('/iqty',passport.authenticate('jwt',{session:false}) ,(req,res)=>{
  const price = Number(req.body.price);
  const Index = req.body.index;
  Cart.findById({_id:req.user.id},(err, cart)=>{
    if (err) {console.log(err);}
    cart.totalQty +=1;
    cart.totalPrice += price;
    cart.selectedProduct[Index].qty += 1;
    cart.selectedProduct[Index].price += price;
    Cart.findByIdAndUpdate({_id:req.user.id},{$set:cart}, {new:true},(err,done)=>{
        if (err) {console.log(err);}
...
//---------------------------------------------|
router.post('/dqty',passport.authenticate('jwt',{session:false}) ,(req,res)=>{
  const price = Number(req.body.price);
  const Index = req.body.index;
  Cart.findById({_id:req.user.id},(err, cart)=>{
    if (err) {console.log(err);}
...
//---------------------------------------------|
router.post('/vqty',passport.authenticate('jwt',{session:false}) ,(req,res)=>{
...
//              DELETE CART ITEM               |
//---------------------------------------------|
router.post('/pro',passport.authenticate('jwt',{session:false}) ,(req,res)=>{
...
router.post('/clear',passport.authenticate('jwt',{session:false}) ,(req,res)=>{
  Cart.findByIdAndDelete({_id:req.user.id},(err, cart)=>{
    if (err) {console.log(err);}
